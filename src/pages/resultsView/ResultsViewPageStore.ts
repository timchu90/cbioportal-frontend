...
export class ResultsViewPageStore {
    ...
    @observable.ref
    public _selectedEnrichmentCopyNumberProfile: MolecularProfile;
    ...
    @computed get selectedEnrichmentMutationProfile() {
        if (
            !this._selectedEnrichmentMutationProfile &&
    ...
    @computed get selectedEnrichmentCopyNumberProfile() {
        if (
            !this._selectedEnrichmentCopyNumberProfile &&
    ...
    @computed get selectedEnrichmentMRNAProfile() {
        if (
            !this._selectedEnrichmentMRNAProfile &&
    ...
    @computed get selectedEnrichmentProteinProfile() {
        if (
            !this._selectedEnrichmentProteinProfile &&
    ...
    public initDriverAnnotationSettings() {
        ...
        this.driverAnnotationSettings.driverFilter = !!AppConfig.serverConfig
            .oncoprint_custom_driver_annotation_default;
        ...
        this.driverAnnotationSettings.hotspots = !AppConfig.serverConfig
            .oncoprint_oncokb_hotspots_default;
        (this.driverAnnotationSettings as any)._oncoKb = !AppConfig.serverConfig
            .oncoprint_oncokb_hotspots_default;
        (this.driverAnnotationSettings as any)._ignoreUnknown = !!AppConfig
    ...
    readonly cnSegmentsByChromosome = remoteData<{
        ...
        {
            await: () => [this.genes, this.samples],
            invoke: () => {
                if (this.genes.result) {
                    return Promise.resolve(
                        _.reduce(
                            _.uniq(this.genes.result.map(g => g.chromosome)),
                            (
                                map: {
    ...
    readonly coexpressionTabMolecularProfiles = remoteData<MolecularProfile[]>({
        ...
        invoke: () =>
            Promise.resolve(
                filterAndSortProfiles(this.molecularProfilesWithData.result!)
            ),
    });
    ...
    readonly alteredPatientKeys = remoteData({
        ...
        invoke: () => {
            ...
            return Promise.resolve(
                this.patients
                    .result!.map(s => s.uniquePatientKey)
                    .filter(
                        patientKey =>
    ...
    readonly queriedVirtualStudies = ServerConfigHelpers.sessionServiceIsEnabled()
        ? remoteData({
    ...
    @computed get mutationsByGene(): { [hugeGeneSymbol: string]: Mutation[] } {
        ...
        if (this.mutationsTabShouldUseOql && this.queryContainsMutationOql) {
            ...
            mutations = (this.oqlFilteredAlterations.result || []).filter(a =>
                isMutation(a)
            );
        } else {
            ...
            const patientFilter = {
                uniquePatientKeys: _.uniq(
                    this.samples.result.map(
                        (sample: Sample) => sample.uniquePatientKey
                    )
                ),
            } as PatientFilter;
    ...
    @computed get isQueryInvalid() {
        return (
            this.hugoGeneSymbols.length >
            AppConfig.serverConfig.query_gene_limit
        );
    }
    ...
    readonly customDriverAnnotationReport = remoteData<{
        ...
        onResult: result => {
            initializeCustomDriverAnnotationSettings(
                ...
                AppConfig.serverConfig.oncoprint_oncokb_hotspots_default ===
                    'custom'
            );
        },
    });
    ...
    public putativeDriverAnnotatedMutationCache = new MobxPromiseCache<
        { entrezGeneId: number },
        AnnotatedMutation[]
    >(q => ({
        await: () => [
    ...
    readonly annotatedMolecularData = remoteData<
        AnnotatedNumericGeneMolecularData[]
    >({
        await: () => [
            this.molecularData,
        ...
        invoke: () => {
            ...
            return Promise.resolve(
                this.molecularData.result!.reduce(
                    (acc: AnnotatedNumericGeneMolecularData[], next) => {
                        ...
                        if (!ignoreUnknown || d.oncoKbOncogenic) {
    ...
    readonly mutationEnrichmentData = makeEnrichmentDataPromise({
        ...
        fetchData: () =>
            internalClient.fetchMutationEnrichmentsUsingPOST({
                molecularProfileId: this.selectedEnrichmentMutationProfile
                    .molecularProfileId,
                enrichmentType: 'SAMPLE',
                enrichmentFilter: {
                    alteredIds: this.alteredSamples.result.map(s => s.sampleId),
                    unalteredIds: this.unalteredSamples.result.map(
                        s => s.sampleId
                    ),
                },
            }),
    ...
    readonly copyNumberHomdelEnrichmentData = makeEnrichmentDataPromise({
        ...
        fetchData: () =>
            this.getCopyNumberEnrichmentData(
    ...
    private getCopyNumberEnrichmentData(
        ...
        return internalClient.fetchCopyNumberEnrichmentsUsingPOST({
            molecularProfileId: this.selectedEnrichmentCopyNumberProfile
                .molecularProfileId,
    ...
    readonly mRNAEnrichmentProfiles = remoteData<MolecularProfile[]>({
        await: () => [this.molecularProfilesInStudies],
        invoke: () => {
            const mrnaProfiles = this.molecularProfilesInStudies.result!.filter(
                p => {
                    return (
    ...
    readonly mRNAEnrichmentData = makeEnrichmentDataPromise({
        ...
        fetchData: () =>
            internalClient.fetchExpressionEnrichmentsUsingPOST({
                molecularProfileId: this.selectedEnrichmentMRNAProfile
                    .molecularProfileId,
                enrichmentType: 'SAMPLE',
                enrichmentFilter: {
                    alteredIds: this.alteredSamples.result.map(s => s.sampleId),
    ...
    readonly proteinEnrichmentProfiles = remoteData<MolecularProfile[]>({
        await: () => [this.molecularProfilesInStudies],
        invoke: () => {
            const protProfiles = this.molecularProfilesInStudies.result!.filter(
                p => {
                    return (
                        p.molecularAlterationType ===
    ...
    readonly proteinEnrichmentData = makeEnrichmentDataPromise({
        ...
        fetchData: () =>
            internalClient.fetchExpressionEnrichmentsUsingPOST({
                molecularProfileId: this.selectedEnrichmentProteinProfile
                    .molecularProfileId,
                enrichmentType: 'SAMPLE',
                enrichmentFilter: {
                    alteredIds: this.alteredSamples.result.map(s => s.sampleId),
    ...
    readonly rnaSeqMolecularData = remoteData<{
        [hugoGeneSymbol: string]: NumericGeneMolecularData[][];
    }>({
        await: () => [
        ...
        invoke: async () => {
            ...
            const data = await this.geneMolecularDataCache.result!.getPromise(
                queries,
                true
            );
            ...
            const keyedByHugoSymbol = _.mapKeys(
                groupedByGene,
                (val, entrezGeneId: string) => {
                    // look up huge gene symbol on gene with matching entrez
                    return _.find(
                        this.genes.result,
                        (gene: Gene) =>
                            gene.entrezGeneId.toString() === entrezGeneId
                    )!.hugoGeneSymbol;
                }
            );

            const unwrapCacheObjects: {
                [hugeGeneSymbol: string]: NumericGeneMolecularData[][];
            } = _.mapValues(
                keyedByHugoSymbol,
                (
                    val: AugmentedData<
                        NumericGeneMolecularData[],
                        GeneMolecularDataCache
                    >
                ) => {
                    return _.map(
                        val,
